...
function mostrarPagina(pagina){
    ...
    MenuItens.style.maxHeight = "0px"; // fecha menu celular
}
...
function renderizarHome(){
    ...
    document.getElementById("destaque-grid").innerHTML =
        destaques.length ? agruparEmLinhas(destaques) : '<p class="sem-itens">Nenhum produto em destaque.</p>';

    document.getElementById("ofertas-grid").innerHTML =
        ofertas.length ? agruparEmLinhas(ofertas) : '<p class="sem-itens">Nenhuma oferta no momento.</p>';

    document.getElementById("catalogo-grid").innerHTML =
        todos.length ? agruparEmLinhas(todos) : '<p class="sem-itens">Nenhum produto cadastrado.</p>';
}
...
function renderizarPedidos(){ // Renderização de pedidos
    var pedidos = getPedidos();
...
function apagarPedido(id){
    ...
    var pedidos = getPedidos().filter(function(p){ return p.id !== id; });
    salvarPedidos(pedidos);
    renderizarPedidos();
}
/*/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
...
function salvarProduto(){
    ...
    if(id){
        ...
        produtos = produtos.map(function(p){
            if(p.id === parseInt(id)){
                return { id: p.id, nome: nome, preco: preco, img: img, alt: alt, categoria: cat };
            }
            return p;
        });
    }else{
        ...
        var novoId = Date.now();
        produtos.push({ id: novoId, nome: nome, preco: preco, img: img, alt: alt, categoria: cat });
    }
...
function apagarProduto(id){
    ...
    var produtos = getProdutos().filter(function(p){ return p.id !== id; });
    salvarProdutos(produtos);
    renderizarAdmin();
}
