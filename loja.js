...
var PRODUTOS_KEY = "laco_linha_produtos"

function getProdutos() {
  var salvo = localStorage.getItem(PRODUTOS_KEY)
  if (salvo) {
    try {
  ...
  var iniciais = [
    ...
    {
      id: 9,
      nome: "Tapete pequeno florido (Marrom/Branco)",
      preco: 24.99,
  ...
  salvarProdutosNoStorage(iniciais)
  return iniciais
}

function salvarProdutosNoStorage(lista) {
  localStorage.setItem(PRODUTOS_KEY, JSON.stringify(lista))
}

var produtos = getProdutos() // Carrega na memória ao iniciar
...
function renderizarHome() {
  var destaques = produtos.filter(function (p) {
    return p.categoria === "destaque"
  })
  var ofertas = produtos.filter(function (p) {
    return p.categoria === "oferta"
  })
  var todos = produtos
...
function confirmarInteresse() {
  ...
  var quantidade =
    parseInt(document.getElementById("lead-quantidade").value) || 1
  ...
  lista.innerHTML = leads
    .map(function (p) {
      return (
        ...
        "</div>" +
        '<button class="btn-apagar" onclick="apagarLead(' +
        p.id +
        ')">&#x1F5D1; Apagar</button>' +
        "</div>" +
        "</div>"
...
function salvarProduto() {
  ...
  salvarProdutosNoStorage(produtos) // Salva a alteração no LocalStorage
  limparFormProduto()
  renderizarProdutos()
}
...
function apagarProduto(id) {
  ...
  salvarProdutosNoStorage(produtos) // Salva a nova lista sem o produto excluído
  renderizarProdutos()
}
